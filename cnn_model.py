...
def kociemba_to_onehot(kociemba_str):
    ...
    onehot_vec = np.concatenate([color_mapping[char] for char in kociemba_str], dtype=np.int8)
    return onehot_vec
...
model.fit(X, y, epochs=100, batch_size=128, validation_split=0.2, callbacks=[early_stopping])
